...
bool face_lighting_check(int inputAlpha) {
    ...
    if (inputAlpha == 254) return false; // Checks for alpha 254, and returns that this face should not be lit. Used for Shade false, non-emissive
...
float remap_alpha(float inputAlpha) {
    ...
    if (inputAlpha == 254) return 255.0; // Checks for alpha 254 and converts all pixels of that to alpha 255. Used for Shade false, non-emissive
